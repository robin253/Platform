...
class T_DDLAUDIT_BATCH_INFO_ADMIN(admin.ModelAdmin):
    list_display = (             
...
class T_DDLAUDIT_BATCH_DETAIL_ADMIN(admin.ModelAdmin):
    list_display = (              #后台展示列表
        'audit_batch','sqlnum','sqltext','sqltype')
    search_fields = ['audit_batch__audit_batch']  #后台搜索框 可以搜索的列   # 查询外键信息 http://blog.csdn.net/davidsu33/article/details/51672163
    list_filter = ['sqltype']     #后台过滤工具栏 用于过滤的列
    list_display_links = ['audit_batch']  # 设置页面上哪个字段可单击进入详细页面
    ordering = ['audit_batch', 'sqlnum']    #后台展示的排序字段 


admin.site.register(T_DDLAUDIT_BATCH_INFO,T_DDLAUDIT_BATCH_INFO_ADMIN)   
admin.site.register(T_DDLAUDIT_BATCH_DETAIL,T_DDLAUDIT_BATCH_DETAIL_ADMIN) 
