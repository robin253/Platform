...
def checkresult(request):
    ...
    for item in groupinfo:
        ...
        for item2 in userinfo:
            list_executor.append(item2.username)
    ...
    if request.method=="POST":
        ...
        if request.POST.has_key('batchcheck'):
            ...
            try:
                audit_detail=models.T_DMLAUDIT_BATCH_INFO.objects.get(audit_batch=audit_batch)
                re_detail=audit_detail.t_dmlaudit_batch_detail_set.all()
            ...
            audit_detail=models.T_DMLAUDIT_BATCH_INFO.objects.filter(created_at__range=(install_data_from,install_data_to)).order_by('-created_at')
            ...
            return render(request,"dmlaudit/checkresult.html",returninfo)
